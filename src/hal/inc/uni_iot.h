...
static inline uni_bool uni_is_leap_year(TimerDate *date) {
...
static inline long uni_get_clock_time_ms(void) {
  struct timespec ts;
...
#define WLAN  "wlan0"
static inline uni_s32 uni_get_mac_address(uni_u8 *address, uni_s32 len) {
  uni_s32 sock, ret;
  struct ifreq ifr;
  ...
  if ((ret = ioctl(sock, SIOCGIFHWADDR, &ifr, sizeof(ifr))) == 0) {
    uni_memcpy(address, ifr.ifr_hwaddr.sa_data,
               uni_min(len, sizeof(ifr.ifr_hwaddr.sa_data)));
    ret = 0;
  } else {
    printf("%s%d: get mac failed\n", __FUNCTION__, __LINE__);
    ret = -1;
  }
  close(sock);
  return ret;
