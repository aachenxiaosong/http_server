...
int _push_into_list(unsigned char *buf, int n, list_head *list,
  ...
  int i = 0;
  node_t *node;
  node = (node_t *)uni_malloc(sizeof(node_t));
  memset(node, 0x0, sizeof(node_t));
  for (i = 0; i < n; i++) {
    node->data[i] = buf[i];
  }
  node->size = n;
  LOGT(SERIAL_TAG, "push into %s list", list == &g_serial.send_list ? "send" : "recv");
  for (i = 0; i < n; i++) {
    printf(" %02x ", buf[i]);
  }
  printf("\n");
...
int _pull_from_list(unsigned char *buf, int *n, list_head *list,
  ...
  *n = node->size;
  uni_free(node);
  LOGT(SERIAL_TAG, "pull from %s list", list == &g_serial.send_list ? "send" : "recv");
  for (i = 0; i < *n; i++) {
    printf(" %02x ", buf[i]);
  }
  printf("\n");
...
static void _deal_with_recv_byte(char recv_byte) {
  if (g_serial.recv_cache_index == 0) {
    ...
    if (recv_byte == 0x03) { //tail
      ...
      LOGE(SERIAL_TAG, "serial recv tail failed, char = %d", recv_byte);
    }
...
static int _uart_send(unsigned char *buf, int n) {
  ...
  LOGT(SERIAL_TAG, "serial send %s >>>>>>>>>>>>>>>>>>>", rc < 0 ? "failed" : "succeed");
  for (i = 0; i < n; i++) {
    printf("%02x ", buf[i]);
  }
  printf("\n");
  ...
  while (1) {
    while (_pull_from_list(data, &n, &g_serial.send_list,
      ...
      if (_uart_send(data, n) < 0) {
        LOGE(SERIAL_TAG, "uart send error!");
      }
    }
...
static int _init_uart() {
  ...
  if (-1 == g_serial.uart_fd) {
    LOGE(SERIAL_TAG, "open %s failed", device);
    return -1;
  }
  if (_uart_set_speed() < 0) {
    LOGE(SERIAL_TAG, "uart set speed error !");
    return -1;
  }
  if (_uart_set_parity() < 0) {
    LOGE(SERIAL_TAG, "uart set parity error !");
    return -1;
  }
  return 0;
}
...
static int _init_thread() {
  ...
  if (ret != 0) {
    LOGE(SERIAL_TAG, "_recving_process pthread_create fail!");
    return ret;
  }
  LOGT(SERIAL_TAG, "create recving thread success");
  usleep(500 * 1000);
  ret = pthread_create(&sending_pid, NULL, &_sending_process, NULL);
  if (ret != 0) {
    LOGE(SERIAL_TAG, "_sending_process pthread_create fail!");
    return ret;
  }
  LOGT(SERIAL_TAG, "create sending thread success");
  return 0;
}
...
int serial_init() {
  ...
  LOGT(SERIAL_TAG, "uart inited");
  if (0 != _init_thread()) {
    return -1;
  }
  LOGT(SERIAL_TAG, "serial thread inited");
  return 0;
}
