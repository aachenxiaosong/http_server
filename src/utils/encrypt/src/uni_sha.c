...
static int SHA1Input(SHA1Context *context, const uint8_t *message_array,
                     unsigned length) {
  if (!length) {
    return shaSuccess;
  }
...
* return  0 成功，-1 失败
*/
int SHA1DigestStr(const char *input, char *result) {
  int i, err;
  SHA1Context sha;
  ...
  for (i = 0; i < SHA1HashSize ; ++i) {
    snprintf(&result[i*2], 3, "%02X", (uint32_t)output[i]);
  }
  return SHA1HashSize * 2 + 1;
}
