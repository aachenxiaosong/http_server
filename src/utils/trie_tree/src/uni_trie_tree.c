...
static int _add_node_path(TrieNode *parent, const char *str) {
  ...
  if (NULL == node) {
    node = (TrieNode *)uni_malloc(sizeof(TrieNode));
    if (NULL == node) {
      LOGE("trie", "memory malloc failed");
      return -1;
    }
    uni_memset(node, 0, sizeof(TrieNode));
    node->parent = parent;
...
int TrieTreeAdd(TrieTree *tree, const char *str) {
  if (-1 == _check_str(str)) {
    LOGE("trie", "invalid word for trie tree");
    return -1;
  }
...
int TrieTreeDelete(TrieTree *tree, const char *str) {
  ...
  if (NULL == node) {
    LOGW("trie", "trie node does not exists");
    return -1;
  }
  node->active = FALSE;
...
int TrieTreeRead(TrieTree *tree, const char *str, int *value) {
  TrieNode *node = _find_node(tree, str);
  if (NULL == node) {
    LOGD("trie", "trie tree read failed, key=%s", str);
    return -1;
  }
  *value = node->value;
  return 0;
...
TrieTree* TrieTreeCreate(void) {
  TrieTree *tree = (TrieTree *)uni_malloc(sizeof(TrieTree));
  uni_memset(tree, 0, sizeof(TrieTree));
  tree->parent = NULL;
  tree->index = -1;
  return tree;
}
