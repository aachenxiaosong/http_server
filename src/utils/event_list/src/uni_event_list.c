...
EventListHandle EventListCreate(EventHandler event_handler) {
...
static void _worker_thread_exit(EventList *event_list) {
  ...
  uni_pthread_destroy(event_list->pid);
}
...
static void _traverse(EventList *event_list, EventHandler traverse_handler) {
  EventListItem *p;
  uni_pthread_mutex_lock(event_list->mutex);
  list_for_each_entry(p, &event_list->highest_list, EventListItem, link) {
    if (traverse_handler) {
      traverse_handler(p->event);
    }
  }
  list_for_each_entry(p, &event_list->medium_list, EventListItem, link) {
    if (traverse_handler) {
      traverse_handler(p->event);
    }
  }
  list_for_each_entry(p, &event_list->lowest_list, EventListItem, link) {
    if (traverse_handler) {
      traverse_handler(p->event);
    }
  }
