...
RiliBookLiftMessageHandler :: RiliBookLiftMessageHandler() : ILiftCtrlMessageHandler("rili_book_lift_handler")
{
}
...
LiftCtrlMessageRsp* RiliBookLiftMessageHandler :: handle(const LiftCtrlMessageReq &request)
{
    ...
    string unlock_floors = open_floor;//for rili, no open floor supported, so add it to unlock list
    ...
    string up_down = RILI_UP;
    int ret;
    if (req.mode().compare(req.MODE_NONE) == 0)
    {
        unlock_floors = "";
        ret = rili_lift_ctrl.bookElevator(i_cluster_id, from_floor, up_down, unlock_floors, req.unlockTime(), wl_response);
    }
    else if (req.mode().compare(req.MODE_OPEN) == 0)
    {
        ret = rili_lift_ctrl.reserveElevator(i_cluster_id, from_floor, open_floor, wl_response);
    }
