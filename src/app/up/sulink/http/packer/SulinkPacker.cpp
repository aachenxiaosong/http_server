...
SulinkPacker :: SulinkPacker() : ISulinkPacker("sulink_packer") {}
...
ISulinkMessage* SulinkPacker :: unpack(const string& raw_data) {
    if (checkPackHeader(raw_data) != 0) {
        return NULL;
    }
    string message_type;
    CJsonObject(raw_data)["header"].Get("method", message_type);
    if (message_type.compare("device-ubox-lift-info") == 0) {
...
string* SulinkPacker :: pack(const ISulinkMessage &message) {
    string *data = NULL;
    switch (message.type()) {
