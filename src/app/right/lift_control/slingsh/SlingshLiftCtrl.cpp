...
SlingshLiftCtrl :: SlingshLiftCtrl(const string& url)
{
    Poco::URI uri(url);
    mIp = uri.getHost();
    mPort = uri.getPort();
...
int SlingshLiftCtrl :: tcpSend(unsigned char *request, int request_len, unsigned char *response, int *response_len) {
  int sockfd;
  struct sockaddr_in server_addr;

  if ((sockfd = socket(PF_INET, SOCK_STREAM, 0)) == -1)
  {
    LOGE(SLINGSH_LIFT_CTRL_TAG, "get socket failed");
    return -1;
  }
